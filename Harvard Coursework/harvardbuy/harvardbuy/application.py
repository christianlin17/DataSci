...
def homepage():
    ...
    # Create a list of ids from the dictionary
    out = []
    for ele in ids:
        out.append(ele['id']) 
    # print(out)
    ...
    ################ SECOND RECTANGLE ################
    second = db.execute("SELECT id FROM products WHERE price < 50")
    print(second)
...
def sell():
    ...
    user_email = db.execute("SELECT email FROM users WHERE id = :placeholderid", placeholderid = id_user)
    # print(user_email)
    email_temp = user_email[0]['email'] + '@college.harvard.edu'
    # print(email_temp)

    if request.method == "POST": # data has been typed

        # Get all the data from the sell now page
        ...
        rows = db.execute("INSERT INTO products (user, name, location, delivery, price, description, category, negotiable, picture1, picture2, email) VALUES (:user, :product_name, :item_location, :delivery_choice, :price, :item_description, :categories, :negotiable, :picture1, :picture2, :email)", user=id_user, product_name=product_name, item_location=item_location, delivery_choice=delivery_choice, price=price, item_description=item_description, categories=categories, negotiable=negotiable, picture1=filepath, picture2=filepath2, email = email_temp)
...
def register():
    ...
    if request.method == "POST": # data has been typed
        ...
        # Checks if email already exists, and if yes, prints error
...
def category_selector(category):
    ...
    # If there is at least one item that matches the category
    if len(selection) > 0:
        ...
        # If there are no items that match the category, then error
...
def other_misc():
    return category_selector("misc-other")
